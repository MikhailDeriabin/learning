...
public class MaxBinaryHeapArray implements PriorityQueue {
    private int nextElemIndex;
    private final int maxLength;

    private final int[] nodes;

    public MaxBinaryHeapArray(int maxLength) {
    ...
    private void reorderDown(int parentNodeIndex) {
        ...
        if(rightChildIndex < nextElemIndex && nodes[maxNodeIndex] < nodes[rightChildIndex]){
            maxNodeIndex = rightChildIndex;
        }

        if(maxNodeIndex != parentNodeIndex){
            swap(parentNodeIndex, maxNodeIndex);
            reorderDown(maxNodeIndex);
        }
    }
